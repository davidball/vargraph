...
from mapping.pathway_matrix.pathway_matrix_by_genes import *
...
class PathwayMatrixByGenesStatic(PathwayMatrixByGenes):
    ...
    def pathway_data_to_hash(self):
        if hasattr(self, "pathway_data"):
            ...
            for n in nodes:
                print(n)
                key = n['reactome_pe_stid']
                node_val = n  # {k: v for k, v in n.items()}
                node_val['id'] = n['reactome_pe_stid']  # n.id
                node_val['labels'] = n['pathway_name']  # l for l in n.labels]
                result[key] = node_val
            return result
        else:
            return {}
